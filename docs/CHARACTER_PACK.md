...
- PNG 형식을 사용한다.
- 투명 배경을 권장한다.
- 모든 모션의 모든 프레임은 첫 `idle` PNG와 같은 캔버스 크기를 사용한다. 로더는 픽셀을 디코딩하지 않고 이미지 크기만 확인해 다른 크기의 팩을 실행 전에 거부한다.
...
현재는 프레임 경로를 스캔하고 각 PNG의 크기를 확인한 뒤, 실제 표시할 이미지 파일만 디코딩한다. 모든 애니메이션을 한 번에 메모리에 올리지 않는다.

### 작업용 스프라이트시트와 실행용 PNG

여러 동작을 한 장의 스프라이트시트로 제작·검수해도 된다. 다만 **현재 엔진은 스프라이트시트나 WebP를 직접 로드하지 않는다.** 사용할 프레임을 각각 투명 PNG로 추출해 위의 모션 폴더에 넣어야 한다. 그림 제작용 시트의 행 이름과 엔진의 모션 이름이 다르면 실제 지원 모션으로 연결할 방법을 먼저 확인한다.
...
현재 `idle`·`walk`를 자동 행동으로 재생한다. 클릭 한 번은 `happy`, 더블클릭은 `special`, 드래그를 시작하면 `dragged`의 첫 프레임, 놓으면 `fall` 다음 `land`를 한 번씩 재생한 뒤 `idle`로 돌아간다. 단일 클릭 반응은 시스템의 더블클릭 판별 시간이 지난 뒤 시작한다. 드래그는 시스템의 최소 이동 거리보다 멀리 움직였을 때 시작된다.

해당 PNG가 없으면 `character.toml`의 fallback 또는 기본 `idle`이 사용된다. 기본 예제 팩에는 반응 이미지가 없어 입력은 처리되지만 그림 변화는 보이지 않는다. `look`·`sleep`·`wake`·`sit`·`stretch` 등의 행동 연결은 이후 단계에서 구현한다.

현재 이동은 `default_facing` 방향으로 만든 `walk` 프레임을 반대 방향에서 좌우 반전한다. 별도의 왼쪽·오른쪽 PNG 폴더를 선택하는 기능은 아직 없다. `look` 폴더도 순차 프레임 재생용이며, 마우스 위치에 따라 방향 이미지를 선택하는 기능은 아직 없다.
